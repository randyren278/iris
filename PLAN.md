...
- **A3:** ~~A Remote Control link/URL is obtainable from outside the session. **Believed false or undocumented.**~~ → **RESOLVED AT CP-1.3: TRUE.** Claude Code writes `~/.claude/sessions/<pid>.json` per live session; when started with `--remote-control` that file gains a `bridgeSessionId` a few seconds after the bridge connects, and the link is `https://claude.ai/code/<bridgeSessionId>`. The id is NOT in `claude agents --json`. Retrieval implemented in `spikes/rc_link.py`, which polls with a deadline and exits 1 with a diagnostic if the field never appears. **P4.5 can therefore text a tappable link rather than a session name.** Caveat: verified by the spike agent with pasted evidence, but not independently reproduced by the orchestrator, because `--remote-control` requires a TTY and non-TTY launches fall into `--print` mode. Treat the field as an undocumented internal that a future release may remove — `rc_link.py`'s exit-1 path is the fallback, and P4.5 must handle it.
